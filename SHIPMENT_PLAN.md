...
| 2.4 | vLLM real-hardware verification | Blocked — risk accepted | 2026-07-22 | Needs access to real enclave/GPU hardware, which won't exist before ship. User accepted this risk directly on 2026-07-22 rather than block ship on it; offset by `RUNBOOK.md`'s new "Enclave Go-Live: Accepted Risks & First-Real-Run Playbook" section. See Section 3 and `## 6. Progress log`. |
...
**Status: Done (2026-07-22)** — all three parts below fixed, plus `.env.enclave.example` (the fourth, separate deliverable this section also calls for), created the same day; see `## 6. Progress log`.
...
## 3. Acceptable risk for v1.0

- **[new, accepted 2026-07-22 by the user directly]** **Section 2.4 (vLLM real-hardware verification) ships unverified.** No real GPU/vLLM hardware exists to test against before this v1.0 goes into the actual air-gapped enclave, and that won't change before ship — accepted rather than blocked on. Everything that *could* be verified without real hardware was (Postgres/migrations/grants, the image contract, the healthcheck fix, JSON-parsing logic, a full app-level flow with the LLM mocked, backup/restore against real production data — see Section 1.3's and 2.5's progress log entries). Narrowed further the same day: `scripts/airgap-install.sh`'s Phase F now automatically checks streaming SSE shape, the `LLM_API_KEY` failure-mode, and concurrency/latency against whatever real `LLM_BASE_URL` is configured at install time — these three were manual operator steps in the original playbook, now they're install-time gates that fail loudly instead of relying on someone remembering to check (see the engineering-follow-up progress log entry below). What's left genuinely unverified: this is still the *first* time those checks run against real hardware at all, and the app-level AI paths beyond summarize (task extraction, workspace digest) aren't driven against real vLLM by anything in this repo. Offset by `RUNBOOK.md`'s "Enclave Go-Live: Accepted Risks & First-Real-Run Playbook" section.
- **[new, accepted 2026-07-22 by the user directly]** **`scripts/airgap-install.sh` ships without ever having run start-to-finish as one continuous process.** This host doubles as Silent Whisper's live production deployment, so a literal single-process rehearsal was never safe to attempt here, and no other host is available before ship. Every phase's actual logic was independently proven correct against real, isolated infrastructure instead (Section 1.3's progress log) — the residual risk is narrowly cross-phase orchestration under real `docker compose` sequencing, not the phase logic itself. The same day, built the infrastructure a real rehearsal would need (`ENV_FILE` support, overridable host ports — see the engineering-follow-up progress log entry) and attempted the actual rehearsal; it hit a hard networking limitation specific to this sandbox (container↔host routing beyond published ports doesn't work here) that isn't expected to exist on a normally-networked host, so the rehearsal itself is still outstanding, but the tooling to run one is ready. Offset by the same `RUNBOOK.md` playbook: run the real first install with someone watching it live (the script is fail-fast and non-destructive by design), and treat that first run as the rehearsal it never got to have.
...
- [x] **[new]** `.env.enclave.example` exists with `LLM_PROVIDER=vllm` and real placeholder values, no Ollama-flavored defaults. *(Done 2026-07-22 — see `## 6. Progress log`.)*
...
- [ ] vLLM verified against real enclave GPU hardware: completions, embeddings, **[new]** streaming, **[new]** embedding-dimension match, auth header behavior, concurrency under realistic load. *(Risk accepted 2026-07-22, not resolved — no real hardware available before ship. Narrowed the same day: streaming/auth-failure-mode/concurrency are now automated `scripts/airgap-install.sh` Phase F checks against real `LLM_BASE_URL` at install time, not manual operator steps — see Section 3 and `RUNBOOK.md`'s "Enclave Go-Live" playbook.)*
...
- [x] Backend test suite green (`cd backend && npm test`), including audit-chain concurrency and grants tests. *(Done 2026-07-22 — 44/44 suites, 561/561 tests, against `silent_whisper_test`, live `silent_whisper` data confirmed unchanged before/after. See `## 6. Progress log`.)*
- [x] `npm audit --omit=dev` clean in both trees, re-run fresh (not trusted from an old number) — in the **networked staging environment**, not inside the air-gapped enclave, since there's no advisory mirror there. *(Done 2026-07-22 — 0 vulnerabilities in `backend`/`frontend`, both `--omit=dev` and full; `scripts` also checked as a bonus (it's a real deployed tree, copied into the backend image per Section 2.3), also clean. See `## 6. Progress log`.)*
...
### 2026-07-22 — Risk acceptance: Section 2.4 and the full-installer rehearsal ship unverified

The user made an explicit go/no-go call: this v1.0 is going into the air-gapped enclave before either Section 2.4 (real vLLM hardware verification) or a literal single-process run of `scripts/airgap-install.sh` on a genuinely separate host can happen — neither is achievable before ship (no GPU hardware, no separate host; this session's host doubles as production, discovered during Section 1.3's work). Rather than leave these as open, un-actioned gaps, recorded the decision explicitly and built the operational mitigation the user asked for.

Added two new entries to `## 3. Acceptable risk for v1.0` (renamed from "...unchanged from original punchlist..." since these two are new, not carried over) with the exact narrower risk surface each actually leaves — not "the AI features are unverified" or "the installer is unverified" wholesale, but the specific untested pieces: streaming SSE shape, `LLM_API_KEY` failure-mode, and real-hardware concurrency/timeout tuning for 2.4; cross-phase `docker compose` orchestration (not phase logic, which is proven) for the installer.

Wrote `RUNBOOK.md`'s new "Enclave Go-Live: Accepted Risks & First-Real-Run Playbook" section — the actual mitigation, not just a risk acknowledgment:
- Enumerates precisely what was and wasn't verified, so whoever runs the real install knows the actual risk surface instead of distrusting everything.
- A recommended first-real-run sequence: watch the install live rather than unattended (it's fail-fast and non-destructive by design, so a failure halts cleanly), then manually exercise the three specific untested vLLM surfaces before opening the app to real users — concurrency (fire a few simultaneous AI requests, confirm they queue and complete within `LLM_TIMEOUT_MS` rather than time out; `LLM_MAX_CONCURRENT_REQUESTS` already defaults to `1`, so this is exercising the queue, not raw parallelism), streaming (the single most likely first-contact failure per Section 2.4's own framing — documented the exact fallback: `LLM_STREAMING_ENABLED=false` uses the non-streaming branch of `backend/src/llm/adapters/vllmAdapter.js`, which *was* exercised end-to-end against a mock, unlike the streaming branch), and the `LLM_API_KEY` failure-mode (deliberately wrong key first, confirm clean failure, then the real key — a check that's never been run against anything, mock or real, until the actual go-live).
- A troubleshooting section grounded in what's actually true about the script's failure modes (non-destructive, safely re-runnable, a vLLM-side failure doesn't touch Postgres/migrations) rather than generic reassurance.

Deliberately did not attempt any further code changes to reduce these risks further (e.g., adding the missing negative-auth-key check to `scripts/airgap-install.sh`, or instrumenting Phase F with real-hardware latency logging) — the user's direction was specifically to accept the risk and offset it through documentation, not additional engineering. Both would be reasonable fast-follow candidates if there's appetite for them later.

### 2026-07-22 — Engineering follow-up on the accepted risks

Offered to the user as a menu of concrete, implementable-now options rather than left as "fast-follow candidates" — they picked 4 of 5: the real streaming smoke test, the negative auth-key check, the concurrency/latency check, and a rehearsal-safe port override for `scripts/airgap-install.sh`. (The 5th, broadening the app-level AI smoke test beyond summarize to task extraction/workspace digest, wasn't selected.)

**Phase F (`phase_vllm_models`) now automates three of the manual checks `RUNBOOK.md`'s "Enclave Go-Live" playbook had asked the operator to remember to do by hand**, all against whatever `LLM_BASE_URL` is actually configured at install time — no mock stand-in for these three, by design, since they're specifically the surfaces a mock can't prove:
- **Streaming**: a real `stream:true` request, parsed with the identical SSE logic `backend/src/llm/adapters/vllmAdapter.js`'s `generate()` uses (`data: <json>` lines, `data: [DONE]` terminator). Fails loudly, with a pointer to the `LLM_STREAMING_ENABLED=false` fallback, if the real deployment's streaming shape doesn't match.
- **Auth failure-mode**: if `LLM_API_KEY` is set, a deliberately wrong key is tried first and must be rejected before the real key is used — closes the one item Section 2.4 flagged as never tested against anything, mock or real.
- **Concurrency/latency**: 3 concurrent completion requests fired directly at the real host, actual latencies logged, a `WARN` (not a hard fail) if the slowest exceeds 70% of `LLM_TIMEOUT_MS` — turns "watch for timeouts" from an operator judgment call into a number.

**Found and fixed a real, pre-existing bug while building the auth check**: neither of the script's two existing `/v1/models` calls (Phase A's preflight reachability check, Phase F's model-list fetch) ever sent the `Authorization` header at all — so both would fail against any real vLLM gateway that requires auth on `/v1/models` too, even with the *correct* `LLM_API_KEY` configured. This wasn't something the streaming/auth/concurrency work set out to find; it surfaced immediately on the first isolated test against a mock that actually enforced auth. Fixed by adding the header to both calls.

**Testing approach**: extracted `phase_vllm_models` into an isolated test harness (a copy of the script with the trailing `main "$@"` call removed, sourced directly) and ran it against four different purpose-built mock OpenAI-compatible servers — one well-behaved, one that wrongly accepts a bad key, one that sends non-SSE garbage on a streaming request, one that rejects concurrent requests with 429. Every positive and negative path was exercised for real, not just read for plausibility. The negative-path testing caught a second real bug: the concurrency check's background-subshell `curl ...; rc=$?` pattern let `set -e` (inherited into the subshell) kill it on a nonzero curl exit *before* `rc=$?` ever ran, which then propagated through `wait "$pid"` and aborted the whole script with a bare exit code instead of a clean `FAIL:` message — fixed with an explicit `if curl ...; then rc=0; else rc=$?; fi`.

**Rehearsal-safe port override**: parameterized `docker-compose.yml`'s three host port bindings (`POSTGRES_HOST_PORT`/`BACKEND_HOST_PORT`/`FRONTEND_HOST_PORT`, defaults unchanged at 5433/8101/3101 — confirmed via `docker compose config` that normal/live behavior is byte-for-byte identical when unset) and added `ENV_FILE` support to `scripts/airgap-install.sh` (defaults to `.env`, but can point at a separate rehearsal env file via `--env-file` on every `$COMPOSE` call) — together with setting `COMPOSE_PROJECT_NAME` to a distinct project name, this is what a true single-process rehearsal on this host, alongside the live production containers, would need.

**Attempted the actual full rehearsal and hit a hard limit specific to this sandbox, not the design**: built real proof image tars via `scripts/build-release-images.sh` (further re-confirming Section 1.2's contract), then tried to stand up a mock vLLM reachable by *one* `LLM_BASE_URL` value from both the bare host process (which is what Phase A/F's `curl` calls run as) and from inside the eventual backend container (Phase G). Confirmed empirically that in this environment: container→host routing via a Docker bridge network's own gateway IP times out, and host→container routing via a container's raw bridge IP gets "no route to host" — only container↔container (same network) and host→container-via-published-port actually work here. That combination makes a single shared `LLM_BASE_URL` address unreachable from both sides at once, which blocks a true unmodified single-process run in this specific sandbox regardless of `.env`/port configuration. This is very likely a deliberate network-isolation property of this coding sandbox, not something a real enclave host would have (a real `LLM_BASE_URL` is a normal, routable network address reachable identically from the host and its containers) — so it's not evidence the script itself has a networking design flaw, just that this particular host can't host the rehearsal. Reported this to the user rather than forcing a workaround (e.g., routing the installer's own host-side checks through a throwaway container using a new, not-pre-staged image, which would break the air-gap image contract) without asking first. All rehearsal test artifacts (mock server, pre-created network, proof image tars/tags) cleaned up afterward; the port-override and `ENV_FILE` infrastructure itself was kept, since it's real, safe (doesn't touch default behavior), and is exactly what a rehearsal on a properly-networked host — even a spare machine, whenever one exists — would need.

Net effect on the accepted-risk items in `## 3. Acceptable risk for v1.0`: the vLLM real-hardware risk is narrower now — streaming, auth failure-mode, and concurrency/latency are automated install-time gates instead of operator-remembered manual steps, so a real deployment either passes them automatically or fails loudly with an actionable message, rather than silently shipping a misconfiguration. The installer-rehearsal risk is unchanged in substance (still needs a genuinely separate, normally-networked host) but the infrastructure to actually run one is now in place and only needs that host to exist.

### 2026-07-22 — Section 2.1c's fourth deliverable: `.env.enclave.example`

Created `.env.enclave.example` at repo root, the deliverable this section always called for separately from the root `.env.example` reconciliation but that had been left outstanding across several earlier passes (flagged in this doc at least three times before now). Every element from this section's spec: `LLM_PROVIDER=vllm`, `LLM_BASE_URL`/`ALLOWED_LLM_ORIGINS` pointing at a real internal vLLM origin placeholder (not the local Ollama container), `VITE_API_URL`/`VITE_WS_URL`/`CORS_ORIGIN` set to an enclave hostname placeholder matching `nginx.enclave.conf.example`'s own placeholder style, and no `GITHUB_PERSONAL_ACCESS_TOKEN`/`HF_TOKEN` lines. Also carries every other key `docker-compose.yml`'s backend service reads (`SILENTWHISPER_VERSION`, task/digest/embedding-worker settings, etc.) so this is a complete, standalone template — not just the LLM-specific subset.

**Caught a real correctness trap before shipping it**: the spec says to use "real `LLM_MODEL`/`EMBEDDING_MODEL`/`EMBEDDING_DIMENSION` placeholders (not `mistral`/`all-minilm`)" — the first draft used `BAAI/bge-base-en-v1.5` at `EMBEDDING_DIMENSION=768` as a plausible-looking, clearly-not-Ollama placeholder. Checked `database/migrations/0009_pgvector_and_embeddings.js` before shipping that and found `message_embeddings.embedding` is a **hardcoded** `vector(384)` column — not driven by `EMBEDDING_DIMENSION` at all. A 768-dim placeholder would have silently modeled an impossible configuration: copying this template and pointing `EMBEDDING_MODEL` at any real 768-dim model without also writing and running a new migration to alter that column type would break on the first real embedding write, in a way nothing in this repo currently catches until Phase F's dimension check (which would correctly fail, but only after an operator had already gotten the wrong idea that EMBEDDING_DIMENSION alone controls this). Fixed by using `BAAI/bge-small-en-v1.5` (a real, different-from-`all-minilm` vLLM-servable model that happens to already produce 384-dimensional output) and adding an explicit comment stating the schema constraint plainly, so the template can't mislead a future reader into thinking this is a free-form knob.

Verified the file mechanically: `grep -c "your_.*_here"` finds the expected placeholders (confirming `scripts/airgap-install.sh`'s own preflight check would correctly refuse an unfilled copy of this template, exactly as intended); confirmed no dead-token lines; confirmed every env var `scripts/airgap-install.sh`'s header comment lists as required is present; sourced the file directly (`set -a; source .env.enclave.example; set +a`) and confirmed it loads without any shell syntax errors.

### 2026-07-22 — Backend test suite (Section 5 checklist)

Ran `cd backend && npm test` for the first time this session. Confirmed the safety net was actually intact before running anything, per `CLAUDE.md`'s explicit warning about `resetDb.js` unconditionally deleting every user: `backend/package.json`'s `test` script hardcodes `PGDATABASE=silent_whisper_test` at the shell level (before Node starts, so it wins over whatever `backend/.env` sets), and `silent_whisper_test` already existed on this host's Postgres instance with all 22 migrations applied (matching the live `silent_whisper` database's own migration count) — no setup needed.

**First attempt was inconclusive, not just slow**: ran `timeout 300 npm test 2>&1 | tail -150` in the background; it was killed by the 300s timeout with the only captured output being the shell's own "Terminated" line — zero jest output at all. Root cause, confirmed by checking `PROJECT_PLAN.md` Section 11 after the user flagged "we've had this issue with jest before": a documented, pre-existing, non-fatal issue where the Jest process doesn't exit on its own after the test run completes (leftover async handles — a "Jest did not exit one second after the test run..." warning, previously seen and already recorded as unrelated to actual test correctness). Piping through `tail -150` made this much worse than it needed to be: `tail` (without `-f`) only ever prints once its input reaches EOF, so as long as the process hangs, `tail` shows *nothing*, not even output produced in the first few seconds — a plain redirect would have at least shown the actual pass/fail summary once the (still-hanging) process's stdout had it buffered.

**Second attempt fixed both problems**: redirected stdout directly to a log file (no `tail` in the pipe) and added Jest's own `--forceExit` flag (`npm test -- --forceExit`) so the process terminates itself once the run finishes instead of hanging — this is exactly what that flag exists for, and turns the known issue into a non-issue rather than something to work around per-invocation. Result: **44/44 test suites, 561/561 tests passed**, actual run time 73.8s. The `console.error` lines visible in the log (`Unknown job_type: BOGUS`, `Ollama embedding request failed: connect ECONNREFUSED`) are expected output from tests that deliberately exercise the message-side-effects and embedding workers' own failure/retry paths, not test failures — the suite's own summary block confirms zero failures. Confirmed the live `silent_whisper` database's row counts (14 users, 84 messages, 163 audit_logs — same as this session's earlier backup/restore work) were unchanged before and after the run.

### 2026-07-22 — `npm audit` (Section 5 checklist)

Ran fresh (not trusted from an old number, per this item's own instruction) in the networked staging environment this host actually is — `cd backend && npm audit --omit=dev` and the same in `frontend`: **0 vulnerabilities in both**. Also ran without `--omit=dev` in both (informational, not this checklist item's stated scope, but cheap to check) — still 0. As a bonus, also ran it in `scripts/`, which isn't one of this item's named "both trees" but is a real dependency tree that ships inside the backend image as of Section 2.3 — also 0 vulnerabilities. Last item on Section 5's checklist that wasn't either Done or a formally accepted risk.
