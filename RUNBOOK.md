...
1. **Preflight** — confirms an existing install is actually running, reads the *currently running* version straight off the live backend's own `GET /health` at `http://localhost:${BACKEND_HOST_PORT:-8101}` (not off any config file, so it can't be fooled by a `.env` that was edited but never actually applied), confirms the target version in `.env` is genuinely different, enforces the `MAJOR`-bump confirmation gate above, and confirms the new image tars are staged. **One expected exception**: `v1.0.0` predates `GET /health` reporting a version at all (that field shipped in `1.1.0`), so the very first upgrade any real enclave ever runs will find no version to read — the script requires `ASSUME_PREVIOUS_VERSION=1.0.0` explicitly in that one case rather than guessing, the same "fail closed, say it out loud" posture `CONFIRM_MAJOR_UPGRADE` uses.
...
8. **Post-upgrade data repair** — runs `scripts/backfill-sentiment-scores.mjs` (and any future one-time data-repair script added to this same phase) inside the new backend container, so an enclave upgrading past a version that introduced a new derived-data table doesn't need a separate manual step to catch up on data that predates it. Idempotent (a no-op once already applied) and, unlike every step before it, **does not fail the upgrade** if it errors — it depends on the AI provider being reachable, the same kind of transient cold-start race step 7's own AI-health check already tolerates rather than fails on — the script logs a warning with the exact command to re-run by hand once the provider is confirmed healthy.
9. **Re-verify the audit log hash chain** is still intact.
10. Writes `upgrade-report-<timestamp>.txt` (same shape as the installer's own report) recording the previous version, the new version, and the backup file path.
...
**Rehearsing against an isolated throwaway stack**: same `ENV_FILE`/`COMPOSE_PROJECT_NAME` mechanism `scripts/airgap-install.sh` already supports — point `ENV_FILE` at a separate rehearsal env with its own `COMPOSE_PROJECT_NAME`, `POSTGRES_HOST_PORT`/`BACKEND_HOST_PORT`/`FRONTEND_HOST_PORT` (so it doesn't collide with a real deployment already bound to the defaults on the same host), and `LLM_PROVIDER=disabled` if no vLLM host is available to rehearse against (a legitimate supported value — this exercises the upgrade mechanics and data preservation, not AI functionality).

This has actually been run this way, once, end-to-end (2026-07-24) — not just validated logic-by-logic: a real `v1.0.0` install (built from the `v1.0.0` git tag via a worktree, exactly reproducing what's really in the enclave) seeded with real data through the real API, upgraded to a real `v1.1.0` build via this exact script, on the same host as a live deployment already bound to the default ports. It found two real bugs on the first attempt — both fixed in `1.1.1` (see `CHANGELOG.md`): the script hardcoded port `8101` instead of respecting `BACKEND_HOST_PORT` (would have silently queried the *other*, real instance once the rehearsal remapped its own port to avoid colliding), and preflight had no way to proceed when `/health` reports no version at all, which is exactly `v1.0.0`'s case and therefore guaranteed to happen on every enclave's very first-ever upgrade (fixed with the `ASSUME_PREVIOUS_VERSION` escape hatch documented above). Re-run clean afterward: 22→23 migrations, grants re-verified, all pre-existing data (users, a workspace, a channel, messages, a DM and its message) confirmed unchanged by direct comparison before/after, and the new feature itself proven to work against data that predated it (backdating the pre-existing DM past a newly-set threshold correctly excluded it from the post-upgrade `GET /api/direct-messages`). Full write-up: `PROJECT_PLAN.md` Section 11, "Rehearsing the enclave upgrade script end-to-end" (2026-07-24).

Still worth doing again before the *actual* production enclave's first real upgrade — a mechanism proven once on a rehearsal stack is not the same as proven against that specific enclave's real configuration, and the same "this run *is* the rehearsal, treat it that way" posture the Go-Live playbook above recommends for the installer applies here too: someone watches it live, reads every `PASS`/`FAIL` line, reads the report afterward regardless of outcome.
