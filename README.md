...
Offline-first, workspace-based messaging platform for project teams — organizations and workspaces, Slack-style channels, direct messages, ad-hoc group DMs, deep threaded replies, server-side presence, an immutable hash-chained audit log, semantic (pgvector) search, a double-bracket (`[[Entity]]`) knowledge graph with a workspace-wide Knowledge Explorer, and configurable local LLM-powered AI utilities (channel summaries, thread action-item extraction, entity summaries, cross-channel "Catch Me Up" digests).
...
**Per-release history of what shipped and what an enclave upgrade will touch**: see [`CHANGELOG.md`](./CHANGELOG.md).
...
**Messaging UX**: display names as the primary identity (usernames as secondary), basic Markdown formatting, inline Markdown checkbox tasks (`- [ ] ... [owner:: @user]`) backed by a live workspace task dashboard, iMessage-style bubbles in DMs vs. left-aligned grouped/avatared messages with visible authorship in channels, @mention autocomplete with browser + in-app notifications, ephemeral direct messages/group DMs via per-user auto-archive, semantic message/channel search via pgvector, and a persistent notification panel covering both mentions and pending membership invitations.

**Knowledge graph**: a double-bracket (`[[Entity Name]]`) entity registry with autocomplete and clickable entity detail pages — editable owner/status/tags/reference-URL metadata, a small typed relationship graph between entities, and an AI-generated "What we know" summary with citations — plus a workspace-level Knowledge Explorer surfacing trending entities, subject-matter-expert discovery, citation history (including thread context), and cross-channel "View Context" navigation, all built on the same authorization join so a private channel's content never leaks into a caller's aggregate view.

**AI (local LLM)**: channel summarization, thread action-item extraction, entity "What we know" summaries, and a cross-channel "Catch Me Up" workspace digest, all served through a provider-adapter interface (Ollama in this test environment, vLLM on the target GPU-backed network) with per-user rate limiting, a global concurrency cap, streamed rendering, and full audit coverage. Actions are surfaced through a scoped "AI Actions" menu rather than always-visible pills.

**Admin & operations**: a dedicated Admin hub (AI settings with live provider health, audit dashboard with a live integrity-verification button, workspace user management, org management, System Admin panel for account/org provisioning and disable/enable), an Admin Analytics dashboard (activity/engagement, dormant channels, collaboration structure, aggregate sentiment trends), system admins can structurally manage any workspace's channels and membership without being a member — structure only, never message content — workspace settings consolidated into one sheet, self-service workspace discovery/subscribe and self-service leave (workspace/org/channel), confirmation dialogs for destructive/high-impact actions (archive, transfer ownership, remove member, revoke invitation, reset password, disable account), actionable workspace-home empty states for new/empty workspaces, and versioned releases with a scripted, data-preserving enclave upgrade path (`CHANGELOG.md`, `scripts/airgap-upgrade.sh`).
