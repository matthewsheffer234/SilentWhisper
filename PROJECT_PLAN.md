...
- **Known gap**: there is no API endpoint to add another user to a workspace. `POST /workspaces/:workspaceId/channels/:channelId/members` adds an *existing* workspace member to a *channel*, but requires the target already be a workspace member — nothing creates that first membership except the workspace's own creation (which only ever adds the creator, as `ADMIN`). Found while provisioning real accounts post-Phase-5 (Section 11); worked around with a direct `workspace_members` insert, which is not something the app itself, or a real admin using it, can do. Needs a `POST /workspaces/:workspaceId/members` (or similar invite/add) route before workspaces are usable by more than one person without direct database access.
...
This is a real product gap, not a test artifact — a real admin using the deployed app has no way to invite anyone else into a workspace they created, which makes multi-user workspaces currently unusable without direct database access. Logged in Section 6 (Core Messaging Utilities) as well. Needs a `POST /workspaces/:workspaceId/members` (or similarly-shaped invite/add) route, with the same admin-only gate `PROJECT_PLAN.md` already uses elsewhere, before this is considered done — not attempted here since it wasn't asked for, and a real feature addition (new endpoint, new authorization rule, new test coverage, and a corresponding audit event) deserves its own deliberate pass rather than being folded into an account-provisioning request.
