...
### 2. Channel and workspace canvases
...
### 3. AI search answers with citations
...
- Rate limiting uses the AI proxy limit, and should reuse the already-shipped global concurrency queue (see "Bounded AI concurrency queue instead of hard-rejecting" in the Done section) rather than introducing a second gate.
...
### 4. Workflow and playbook runs inside channels
...
### 5. Conversation-linked live checklists
...
- Items support title, completion status, optional owner, optional due date, and source message id. Completion/uncompletion should broadcast over WebSocket to current channel/thread viewers. Same terminology call as the already-shipped "Inline Markdown checkbox tasks" entry (Done section): `owner`, not `assignee`.
- This feature should coexist with entry 4: a checklist can later be saved as a playbook template, but v1 should not require the playbook schema to ship first.
- Audit: `CHECKLIST_CREATED`, `CHECKLIST_ITEM_CREATED`, `CHECKLIST_ITEM_UPDATED`, `CHECKLIST_ARCHIVED`; payloads include ids/status transitions and owner ids.
...
- Entity detail pages should show recent references, linked decisions/action items, related entities, and pinned messages/canvases. This connects entry 2 (canvases) if it ships, and can surface the already-shipped "Inline Markdown checkbox tasks" entry's (Done section) owner-tagged tasks as linked action items.
...
### 8. Meeting transcript recap and notes import
...
- AI prompt produces sections for topics, decisions, action items, unresolved questions, and suggested follow-up messages. Accepted action items can flow into the already-shipped "Inline Markdown checkbox tasks" entry (Done section) — posted as a new `- [ ]` line in the relevant channel, not a separate register.
...
### 10. Self-hosted calls or call-notes bridge
...
Design:
- Preferred v1 is not full calls: ship the meeting transcript recap/import flow from entry 8 first. It captures much of the product value with far lower operational risk and fits the current local LLM architecture.
