...
// docs/reviews/2026-07-25-consolidated-meta-review.md finding #1: a summary
// is generated only from channels its generator can read (referencesQuery
// above enforces that), but the stored row itself was workspace-visible to
// any reader once cached — a broader-access member's summary could still
// name a channel a narrower-access reader can't open. Re-checks the
// *reader's* membership against every channel the summary actually cites
// (recorded per-citation at generation time, entities.js's POST .../summary
// handler below) before either exposure point is allowed to return it.
// Returns null (same shape as "no summary generated yet") rather than a 403,
// so a summary the reader can't see is indistinguishable from one that
// doesn't exist yet.
async function readableSummaryFor(userId, summary) {
  if (!summary) return null;
  const citedChannelIds = [...new Set((summary.citations || []).map((c) => c.channelId))];
  if (citedChannelIds.length === 0) return summary;
  ...
  return Number(visibleCount?.count ?? 0) === citedChannelIds.length ? summary : null;
}
...
entitiesRouter.get('/workspaces/:workspaceId/entities/:entityId', async (req, res, next) => {
  try {
    ...
    const visibleSummary = await readableSummaryFor(req.user.id, latestSummary);
    ...
    res.json({
      ...
      summary: serializeSummary(visibleSummary),
    });
  } catch (err) {
    next(err);
  }
...
// otherwise see. The stored row is workspace-scoped (one summary shared by
// every member who can open the entity), so readableSummaryFor() re-checks
// the *reader's* membership against every cited channel before this route
// returns it — a summary generated by a broader-access member can still
// name a channel a narrower-access reader can't open, and the cache must not
// be the thing that reveals it to them.
entitiesRouter.get('/workspaces/:workspaceId/entities/:entityId/ai/summary', async (req, res, next) => {
  try {
    ...
    const visibleSummary = await readableSummaryFor(req.user.id, summary);
    res.json(serializeSummary(visibleSummary));
  } catch (err) {
    next(err);
  }
});
